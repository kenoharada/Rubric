#!/usr/bin/env python3
"""Generate LaTeX ablation study tables comparing configuration variants."""
import argparse
import json
import math
...
def main() -> int:
    ...
    parser.add_argument(
        "--metrics",
        default="qwk", # "qwk,accuracy,spearman,f1_macro,mae"
        help="Comma-separated list of metrics: accuracy,qwk,spearman,f1_macro,mae",
    )
    ...
    for dataset, model_name, run_name, run_dir in iter_runs(root):
        ...
        if not run_dir.exists():
            ...
            continue
        accuracy = load_metric(run_dir / "accuracy.txt")
        qwk = load_metric(run_dir / "qwk.txt")
        spearman = load_spearman(run_dir / "results.jsonl", dataset)
        f1_macro, mae = load_f1_mae(run_dir / "results.jsonl", dataset)
    ...
    for dataset in sorted(grouped.keys()):
        ...
        for model_name in sorted(grouped[dataset].keys()):
            ...
            for run_name in required_runs:
                ...
                rows_for_dataset.append((model_name, method_label, metric_values, dict(best_second)))
        if rows_for_dataset:
