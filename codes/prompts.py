...
ETS_RUBRIC_FOR_3 = "Based on the response\'s content, rate the response on a scale of 1 to 3."
...
EVALUATION_PROMPT = '''You are a rater for writing responses on a high-stakes English language exam for second language learners. You will be provided with a prompt and the test-taker's response. Your rating should be based on the rubric below, following the specified format.
...
EVALUATION_PROMPT_ASAP = '''You are a rater for student essays. You will be provided with an essay prompt and a student essay. Your rating should be based on the rubric below, following the specified format.
...
PROMPT_FOR_EVOLUTION_WITH_RATIONALE = """I provided an assistant with the following rubrics to perform an essay grading task for me:
```
{current_rubric}
```

The following are examples of different inputs to the assistant, the rationales for scores from the assistant, the scores from the assistant, and desired scores which I would like the assistant to achieve.
```
{examples}
```
...
Read all the assistant responses and reflect on the rationales given by the assistant. Identify any patterns or common themes in the rationales that led to correct or incorrect ratings. Consider how the rubrics could be adjusted to better align with these patterns by providing clearer/detailed guidelines for the assistant to follow.

Provide the new rubrics within ``` blocks."""


PROMPT_FOR_EVOLUTION_WITHOUT_RATIONALE = """I provided an assistant with the following rubrics to perform an essay grading task for me:
```
{current_rubric}
```
...
Read all the assistant responses and identify any patterns or common themes that led to correct or incorrect ratings. Consider how the rubrics could be adjusted to better align with these patterns by providing clearer/detailed guidelines for the assistant to follow.

Provide the new rubrics within ``` blocks."""

EXAMPLE_FORMAT_ASAP_WITH_RATIONALE = '''Input for the assistant:
Essay to be rated:
"""{response}"""
Rationale from the assistant:
"""{rationale}"""
...
EXAMPLE_FORMAT_ASAP_WITHOUT_RATIONALE = '''Input for the assistant:
Essay to be rated:
"""{response}"""
Score from the assistant:
"""{rating}"""
...
EXAMPLE_FORMAT_WITH_RATIONALE = '''Input for the assistant:
Essay Prompt:
"""{essay_prompt}"""
Essay to be rated:
"""{response}"""
...
EXAMPLE_FORMAT_WITHOUT_RATIONALE = '''Input for the assistant:
Essay Prompt:
"""{essay_prompt}"""
Essay to be rated:
"""{response}"""
