#!/usr/bin/env python3
"""Generate a single combined LaTeX table for all datasets (Tables 2-4 merged).

Uses \\resizebox to fit the table within one page width.
"""
import argparse
import json
import math
...
METHOD_LABELS = {
    ...
    "zero_shot_base_expert_False_train100_iteration1_top3_bs4-8-12_mc4": "AutoCalibrate",
...
def build_combined_table(
    all_data: Dict[str, List[Tuple[str, str, Dict[str, Optional[float]], Dict[str, Tuple[Optional[float], Optional[float]]]]]],
    metrics: List[str],
    ...
    col_spec = "l" + "ll" + ("r" * num_metrics)
    lines.append(f"\\begin{{tabular}}{{{col_spec}}}")
    lines.append("\\toprule")

    header = ["Dataset", "LLM", "Rubric written by"]
    if "qwk" in metrics:
        header.append("QWK")
    ...
    for dataset in dataset_order:
        ...
        for i, (model_name, method_label, metric_values, best_second) in enumerate(rows):
            display_model = MODEL_LABELS.get(model_name, model_name)

            if i == 0:
                dataset_cell = f"\\multirow{{{num_rows}}}{{*}}{{{display_dataset}}}"
            else:
                dataset_cell = ""

            row_cells = [dataset_cell, display_model, method_label]
            ...
            # Add a thin rule between model groups within a dataset
            if i < num_rows - 1:
                next_model = rows[i + 1][0]
                if next_model != model_name:
                    lines.append("\\cmidrule(lr){2-" + str(2 + num_metrics + 1) + "}")

    lines.append("\\bottomrule")
    lines.append("\\end{tabular}}")
    lines.append("\\caption{Zero-shot evaluation results on all datasets. "
                  "Bold and underlined values indicate the best and second-best scores "
                  "for each model within each dataset.}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate a single combined LaTeX table for all datasets."
    )
    ...
    dataset_order = [d for d in ["asap_1", "ASAP2", "ets3"] if d in dataset_filter]
    ...
    for dataset in dataset_order:
        ...
        if rows_for_dataset:
            all_data[dataset] = rows_for_dataset

    if not all_data:
        raise SystemExit("No runs found after filtering for required methods.")

    output_text = build_combined_table(all_data, metrics, dataset_order) + "\n"
    if args.output:
