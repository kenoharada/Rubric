...
\maketitle
\begin{abstract}
The performance of Large Language Models (LLMs) is highly sensitive to the prompts they are given. Drawing inspiration from the field of prompt optimization, this study investigates the potential for enhancing Automated Essay Scoring (AES) by refining the scoring rubrics used by LLMs. Specifically, our approach prompts models to iteratively refine rubrics by reflecting on models' own scoring rationales and observed discrepancies with human scores on sample essays. 
\footnote{Optimization and evaluation codes are available at \url{https://anonymous.4open.science/r/ARR2025-1CA3}}
\end{abstract}

\section{Introduction}
Automated Essay Scoring (AES) systems powered by Large Language Models (LLMs) are increasingly expected to provide real-time, scalable feedback for students and alleviate the grading burden on instructors~\citep{mizumoto2023aes,yancey-etal-2023-rating,naismith-etal-2023-automated,pack2024validity}. Typically, these systems employ static, pre-defined rubrics to guide the evaluation. However, it remains an open question whether rubrics designed for human raters are optimal for LLMs. When human raters use a rubric, they often engage in a collaborative calibration process: they score sample essays, discuss discrepancies in their judgments, and refine their shared understanding of the criteria to ensure consistency~\citep{trace2016rubricsnegotiate,ozfidan2022rubricdev,ouyang2022instructgpt,yoo-etal-2025-dress}. This iterative, reflective practice is overlooked in current LLM-based AES, potentially limiting their alignment with human scoring patterns.

Recent studies show that LLMs have abilities to refine their own outputs especially when there are reliable external feedback~\citep{madaan2023selfrefine,kamoi2024selfcorrect}. Prompt optimization techniques leverage these capabilities to update prompts to maximize a targeted metric and show performance improvements in various tasks such as multi-hop reasoning, instruction following and privacy-aware delegation~\citep{khattab2023dspy,opsahl-ong-etal-2024-optimizing,agrawal2025gepa}.

Inspired by these developments and the calibration process of human raters, we propose a iterative refinement approach for rubrics in LLM-based AES.
Specifically, for each model and dataset, using 100 sample essays and scores assigned by human raters, models are prompted to iteratively refine the rubric reflecting on models' own scoring rationales and observed score discrepancies with human scores to mitigate Quadratic Weighted Kappa (QWK) between LLM and human scores. 

Experiments on three datasets (ASAP, ASAP 2.0, and TOEFL11) and three models (\texttt{GPT-5 mini}, \texttt{Qwen3-30B-A3B}, and \texttt{Gemini 3 Flash}) show that iterative rubric refinement can lead to substantial improvements in QWK on some datasets (up to +0.441 on ASAP). Also, we show even from the simplest seed rubric, our method can achieve comparable or better performance than using rubrics written by human experts. However, we also observe that the effectiveness of refinement is dataset- and setting-dependent, with some cases showing performance degradations. These results suggest that while iterative rubric refinement is a promising direction for improving LLM-based AES, it has room for improvement.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/confusion_matrices.pdf}
\caption{Confusion matrices for Qwen3-30B-A3B with the human expert rubric (top) and our refined rubric (bottom) across three datasets. Cell colors indicate row-normalized proportions; numbers show raw counts. Our refined rubric produces predictions more concentrated along the diagonal, indicating better agreement with human annotations.}
\label{fig:confusion}
\end{figure*}
...
\begin{table*}[t]
  ...
  \begin{tabular}{p{2.2cm}ccccc}
    ...
    \textbf{Human-labeled} &
    \textbf{Texts being} &
    \textbf{Source passages used} &
    \textbf{Model-predicted} &
    \textbf{Rationales for} \\
    & \textbf{scores} &
    \textbf{evaluated} &
    \textbf{to compose responses} &
    \textbf{scores} &
    \textbf{predicted scores} \\
    \midrule
    \citet{liu-etal-2024-hd} & $\checkmark$ &  &  &  &  \\
    \citet{lee-etal-2024-unleashing} &  &  & $\checkmark$ &  &  \\
    \citet{xie2024gradelikehuman} & $\checkmark$ & $\checkmark$ &  &  &  \\
    \citet{xu2025activecritics} & $\checkmark$ & $\checkmark$ & $\checkmark$ &  &  \\
    \citet{liu-etal-2024-calibrating} & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ &  \\
    \midrule
    \textbf{Ours} &
    \textbf{$\checkmark$} & \textbf{$\checkmark$} & \textbf{$\checkmark$} & \textbf{$\checkmark$} & \textbf{$\checkmark$} \\
    \bottomrule
  \end{tabular}
  \caption{Used signals to rewrite or refine evaluation rubrics in prior work vs. ours. Our method uniquely uses all signals, including model-generated rationales for model-predicted scores. \textbf{Texts being evaluated} are the essays/answers/responses that receive scores. \textbf{Source passages used to compose responses} are the reference sentences/passages that the writer must consult when producing the evaluated text (e.g., source documents, provided essay themes). \textbf{Rationales} are models' justifications accompanying model-predicted scores.}
...
\paragraph{Models}
We evaluate using three LLMs accessed via the OpenRouter API~\citep{openrouter}: \texttt{GPT-5-mini}~\citep{gpt5}, \texttt{Gemini~3~Flash}~\citep{Google2025GeminiFlash}, and \texttt{Qwen3-Next-80B-A3B-Instruct}~\citep{qwen3-next}. These models span different providers and architectures.

\paragraph{Baselines}
We compare our Reflect-and-Revise method against two baselines. The first is the \textbf{Human Rubric} baseline, which directly uses the original human-authored rubric without any refinement. The second is \textbf{AutoCalibrate}, which follows AutoCalibrate~\citep{liu-etal-2024-calibrating} in performing a single-pass rubric revision using only score mismatches between model predictions and human labels, without incorporating the model's chain-of-thought rationales. Unlike the original AutoCalibrate, which generates the initial rubric from scratch using an LLM, our implementation initializes from the same human-authored rubric as our method.
...
We present the main results with expert seed rubrics (\S\ref{sec:main_results}), results starting from minimal seed rubrics (\S\ref{sec:simplest_seed}), a qualitative analysis of refined rubrics (\S\ref{sec:qualitative}), and a discussion of iterative refinement effects, scoring distributions, and cross-model rubric transferability (\S\ref{sec:discussion}).

\subsection{Main Results}
\label{sec:main_results}

Table~\ref{tab:asap1} reports zero-shot evaluation results on ASAP P1 with expert seed rubrics; results on ASAP 2.0 and TOEFL11 are provided in Tables~\ref{tab:asap2} and~\ref{tab:toefl} (Appendix~\ref{sec:appendix_full_results}). Bold and underlined values indicate the best and second-best scores for each model, respectively.
...
\midrule
Gemini 3 Flash & Human & 0.427 & \underline{0.404} & 0.568 & \underline{0.307} & \underline{0.697} \\
Gemini 3 Flash & AutoCalibrate & \underline{0.452} & 0.390 & \underline{0.597} & 0.288 & 0.710 \\
Gemini 3 Flash & \textbf{Ours} & \textbf{0.576} & \textbf{0.470} & \textbf{0.613} & \textbf{0.330} & \textbf{0.580} \\
\midrule
GPT-5 mini & Human & 0.042 & 0.110 & 0.151 & 0.058 & 1.430 \\
GPT-5 mini & AutoCalibrate & \underline{0.117} & \underline{0.250} & \underline{0.280} & \underline{0.121} & \underline{1.010} \\
GPT-5 mini & \textbf{Ours} & \textbf{0.430} & \textbf{0.470} & \textbf{0.455} & \textbf{0.284} & \textbf{0.570} \\
\midrule
Qwen3-30B-A3B & Human & \underline{0.121} & \underline{0.130} & \underline{0.323} & \underline{0.123} & \underline{1.540} \\
Qwen3-30B-A3B & AutoCalibrate & 0.073 & 0.110 & 0.217 & 0.076 & 1.620 \\
Qwen3-30B-A3B & \textbf{Ours} & \textbf{0.213} & \textbf{0.400} & \textbf{0.349} & \textbf{0.234} & \textbf{0.870} \\
\bottomrule
\end{tabular}
\caption{Zero-shot evaluation results on ASAP P1 with expert seed rubrics. Bold and underlined values indicate the best and second-best scores for each model.}
\label{tab:asap1}
\end{table*}

On ASAP P1, our method achieves the highest QWK across all three models, with improvements over the human rubric ranging from +0.092 (Qwen3-30B-A3B) to +0.388 (GPT-5 mini). The most notable case is GPT-5 mini, where the human rubric yields near-chance agreement (QWK\,=\,0.042), but our refined rubric raises QWK to 0.430. Our method also consistently outperforms AutoCalibrate, confirming the benefit of iterative refinement with chain-of-thought rationales over single-pass revision without rationales.

On ASAP 2.0 (Table~\ref{tab:asap2}), our method achieves the best QWK for GPT-5 mini (0.466) and Qwen3-30B-A3B (0.664), while AutoCalibrate obtains a slightly higher QWK for Gemini 3 Flash (0.651 vs.\ 0.639). On TOEFL11 (Table~\ref{tab:toefl}), results are more mixed: our method achieves the best QWK only for Qwen3-30B-A3B (0.474), while the human rubric and AutoCalibrate perform better for GPT-5 mini and Gemini 3 Flash, respectively. These results indicate that the effectiveness of iterative rubric refinement varies across datasets. The largest improvements tend to occur when the initial human rubric performs poorly for a given model, suggesting that refinement is most beneficial when the default rubric is poorly calibrated for the target model.

\subsection{Refinement from Minimal Seed Rubrics}
\label{sec:simplest_seed}

A key practical question is whether rubric refinement can reduce the burden of manual rubric authoring. To investigate this, we apply our method starting from a \textit{simplest} seed rubric and compare the resulting test QWK against the unrefined human expert rubric. Table~\ref{tab:simplest} reports these results.
...
\begin{tabular}{@{}llrr@{}}
\toprule
\textbf{Dataset} & \textbf{LLM} & \textbf{Simplest} & \textbf{$\Delta$ vs.\ Human} \\
 & & \textbf{+ Ours} & \textbf{Expert} \\
\midrule
ASAP P1 & GPT-5 mini & 0.285 & +0.243 \\
 & Gemini 3 Flash & 0.556 & +0.129 \\
 & Qwen3-30B-A3B & 0.442 & +0.322 \\
\midrule
ASAP 2.0 & GPT-5 mini & 0.377 & +0.019 \\
 & Gemini 3 Flash & 0.649 & +0.036 \\
 & Qwen3-30B-A3B & 0.669 & +0.094 \\
\midrule
TOEFL11 & GPT-5 mini & 0.527 & +0.080 \\
 & Gemini 3 Flash & 0.493 & $-$0.082 \\
 & Qwen3-30B-A3B & 0.334 & $-$0.030 \\
\bottomrule
\end{tabular}
\caption{QWK comparison: rubrics refined from the simplest seed (``Based on the response's content, rate the response on a scale of 1 to 6.'') vs.\ the rubric written by human experts. $\Delta$ denotes QWK improvement over the Human Expert baseline.}
\label{tab:simplest}
\end{table}

On ASAP P1 and ASAP 2.0, rubrics refined from the simplest seed consistently outperform the unrefined human expert rubric across all three models, with improvements up to +0.322 QWK (Qwen3-30B-A3B on ASAP P1). On TOEFL11, results are mixed: GPT-5 mini improves by +0.080, while Gemini 3 Flash and Qwen3-30B-A3B show slight degradations ($-$0.082 and $-$0.030, respectively). Overall, these results demonstrate that our method can, in many settings, recover or exceed the performance of a carefully authored expert rubric starting from only a minimal specification of the score scale. This substantially reduces the need for manual rubric authoring, as the refinement process can automatically discover effective scoring criteria from a small set of labeled examples.

\subsection{Qualitative Analysis of Refined Rubrics}
\label{sec:qualitative}

To understand how refinement changes rubric content, we examine the rubrics produced by our method on ASAP P1. Figure~\ref{fig:rubric_example} shows representative excerpts from a rubric refined by GPT-5~mini starting from the expert seed (full refined rubric is provided in Appendix~\ref{sec:appendix_refined_rubric}).

Comparison between the original expert rubric and the refined rubric reveals three categories of systematic modifications. First, the refined rubric introduces explicit \textit{general notes} that address common scoring pitfalls, such as instructions not to penalize surface-level errors (spelling and grammar) unless they substantially impede comprehension. Second, the refinement adds concrete \textit{decision rules} for distinguishing adjacent score levels---for example, counting the number of distinct examples and treating brief anecdotes as valid evidence. Third, each score point is supplemented with targeted \textit{rater guidance} that provides specific instructions for boundary cases (e.g., ``if there are only general reasons and the examples are mainly restatements, prefer 3 over 4''). These modifications suggest that the refinement process identifies systematic scoring biases---such as over-penalizing mechanical errors or under-valuing brief anecdotes---and addresses them by making implicit evaluation criteria more explicit and actionable for the model.

\begin{figure*}[t]
\centering
\small
\begin{tcolorbox}[colback=blue!3, colframe=RoyalBlue, title={\textbf{(a) General Notes} --- Cross-cutting guidelines added to the preamble}, fonttitle=\small, fontupper=\scriptsize, left=4pt, right=4pt, top=3pt, bottom=3pt]
\textit{``Distinguish surface errors (spelling, grammar, typos) from communicative breakdowns. Surface errors are expected and should only lower a score if they substantially impede meaning, coherence, or readability.''}
\\[2pt]
\textit{``When in doubt between adjacent scores, use the development and specificity of examples as the primary tiebreaker: more distinct, explained examples $\to$ higher score.''}
\end{tcolorbox}
\vspace{3pt}
\begin{tcolorbox}[colback=green!3, colframe=green!50!black, title={\textbf{(b) Decision Rules} --- Concrete heuristics for distinguishing adjacent scores}, fonttitle=\small, fontupper=\scriptsize, left=4pt, right=4pt, top=3pt, bottom=3pt]
\textit{``Count distinct examples: 0--1 distinct examples $\to$ less likely than 4; 2--3 distinct examples briefly explained $\to$ 4; 3+ distinct examples with brief explanation across different domains or backed by named anecdotes/studies $\to$ 5.''}
\\[2pt]
\textit{``Repetition of the same point with minor variation does not equal multiple elaborations. Distinctness matters: different domains, different types of evidence, or separate illustrative anecdotes count as multiple examples; restatements do not.''}
\end{tcolorbox}
\vspace{3pt}
\begin{tcolorbox}[colback=orange!3, colframe=orange!70!black, title={\textbf{(c) Rater Guidance} --- Per-score-point instructions for boundary cases}, fonttitle=\small, fontupper=\scriptsize, left=4pt, right=4pt, top=3pt, bottom=3pt]
\textit{Score Point 3 --- ``Assign 3 when the essay shows a clear position and basic organizational structure but lacks adequate, specific development. If there are only general reasons and the examples are mainly restatements or repeats, prefer 3 over 4.''}
\\[2pt]
\textit{Score Point 5 --- ``Multiple concrete examples across different domains (e.g., health, environment, social/family) each with brief explanation $\to$ prefer 5 rather than 4. Named anecdotes or brief case reports count as concrete examples when they illustrate the claim.''}
\end{tcolorbox}
\caption{Representative excerpts from a rubric refined by GPT-5~mini on ASAP P1 (expert seed), illustrating three categories of modifications: (a)~cross-cutting general notes, (b)~concrete decision rules, and (c)~per-score-point rater guidance. None of these elements appear in the original human-authored rubric.}
...
\paragraph{Effect of iterative refinement}
To analyze the effect of iterative refinement, we track the training QWK of the best candidate rubric at each iteration. Figure~\ref{fig:training_qwk} shows these trajectories for each model on ASAP 2.0. Across all three models, training QWK improves monotonically over iterations, as the selection mechanism retains only the best candidates at each step. The steepest gains occur in the first two iterations, after which the improvements become more gradual. This pattern suggests that early iterations address the most salient scoring errors, while later iterations yield diminishing but still positive returns. Notably, the rate of improvement differs across models: Qwen3-30B-A3B exhibits the largest single-iteration gain, while Gemini 3 Flash shows a more steady progression.

\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig_iterative_refinement_qwk.pdf}
\caption{Training QWK of the best candidate rubric at each iteration on ASAP 2.0. All models show monotonic improvement, with the steepest gains in early iterations.}
\label{fig:training_qwk}
\end{figure*}

\paragraph{Scoring distribution analysis}
To visualize how rubric refinement changes scoring behavior, Figure~\ref{fig:confusion} shows confusion matrices for Qwen3-30B-A3B with the human expert rubric versus our refined rubric across all three datasets. On ASAP~P1, the human rubric causes severe under-scoring: predictions cluster around scores 2--3 regardless of the true score. Our refined rubric shifts the distribution toward the diagonal, concentrating predictions around score~4---the most frequent true score---and dramatically improving accuracy from 13\% to 40\%. On ASAP~2.0, a similar pattern emerges: the human rubric scatters predictions across scores 1--3, while our rubric aligns predictions with the true score distribution, improving accuracy from 27\% to 51\%. On TOEFL11, the human rubric never predicts the ``High'' category, collapsing all predictions into Low and Medium. Our refined rubric recovers the ability to predict all three levels, improving accuracy from 48\% to 61\%, although some Low-scored essays are misclassified as Medium. These patterns suggest that the primary mechanism of improvement is better calibration of score boundaries to match the target distribution.
...
Table~\ref{tab:hyperparams} summarizes the optimization hyperparameters. Our Reflect-and-Revise method runs for $T{=}5$ iterations with rationale-based feedback, while the AutoCalibrate baseline uses $T{=}1$ without rationales. Both methods maintain $K{=}3$ top candidates, use Monte Carlo trials $M{=}1$, error batch sizes $\mathcal{B}{=}\{4,8,12\}$, and $N{=}100$ training samples.
...
\section{Full Results on ASAP 2.0 and TOEFL11}
...
LLM & Rubric written by & QWK & Accuracy & Spearman & Macro-F1 & MAE \\
\midrule
Gemini 3 Flash & Human & 0.613 & 0.440 & \underline{0.657} & 0.270 & 0.610 \\
Gemini 3 Flash & AutoCalibrate & \textbf{0.651} & \underline{0.490} & \textbf{0.697} & \underline{0.308} & \textbf{0.540} \\
Gemini 3 Flash & \textbf{Ours} & \underline{0.639} & \textbf{0.530} & 0.653 & \textbf{0.388} & \underline{0.550} \\
\midrule
GPT-5 mini & Human & \underline{0.358} & \underline{0.350} & \textbf{0.533} & 0.148 & \underline{0.780} \\
GPT-5 mini & AutoCalibrate & 0.277 & \textbf{0.370} & \underline{0.471} & \underline{0.148} & 0.820 \\
GPT-5 mini & \textbf{Ours} & \textbf{0.466} & \textbf{0.370} & 0.458 & \textbf{0.190} & \textbf{0.710} \\
\midrule
Qwen3-30B-A3B & Human & \underline{0.575} & \underline{0.270} & \textbf{0.668} & \underline{0.272} & \underline{0.890} \\
Qwen3-30B-A3B & AutoCalibrate & 0.544 & \underline{0.270} & 0.577 & 0.259 & 0.950 \\
Qwen3-30B-A3B & \textbf{Ours} & \textbf{0.664} & \textbf{0.510} & \underline{0.657} & \textbf{0.381} & \textbf{0.610} \\
\bottomrule
\end{tabular}
\caption{Zero-shot evaluation results on ASAP 2.0 with expert seed rubrics.}
\label{tab:asap2}
\end{table*}
...
LLM & Rubric written by & QWK & Accuracy & Spearman & Macro-F1 & MAE \\
\midrule
Gemini 3 Flash & Human & \underline{0.574} & \underline{0.690} & \underline{0.576} & 0.244 & 0.700 \\
Gemini 3 Flash & AutoCalibrate & \textbf{0.597} & 0.680 & \textbf{0.648} & \textbf{0.636} & \underline{0.320} \\
Gemini 3 Flash & \textbf{Ours} & 0.471 & \textbf{0.710} & 0.513 & \underline{0.565} & \textbf{0.290} \\
\midrule
GPT-5 mini & Human & \textbf{0.447} & \textbf{0.680} & \underline{0.434} & 0.165 & 0.900 \\
GPT-5 mini & AutoCalibrate & \underline{0.419} & 0.620 & \textbf{0.482} & \textbf{0.542} & \underline{0.410} \\
GPT-5 mini & \textbf{Ours} & 0.329 & \underline{0.670} & 0.361 & \underline{0.483} & \textbf{0.350} \\
\midrule
Qwen3-30B-A3B & Human & 0.364 & 0.470 & \underline{0.471} & 0.344 & 0.530 \\
Qwen3-30B-A3B & AutoCalibrate & \underline{0.428} & \underline{0.570} & 0.454 & \underline{0.533} & \underline{0.460} \\
Qwen3-30B-A3B & \textbf{Ours} & \textbf{0.474} & \textbf{0.610} & \textbf{0.480} & \textbf{0.540} & \textbf{0.390} \\
\bottomrule
\end{tabular}
\caption{Zero-shot evaluation results on TOEFL11 with expert seed rubrics.}
\label{tab:toefl}
\end{table*}
...
The following is the full text of the rubric refined by GPT-5~mini on ASAP P1 starting from the expert seed rubric, corresponding to the excerpts shown in Figure~\ref{fig:rubric_example}.
