...
\maketitle
\begin{abstract}
Large Language Models (LLMs) are increasingly used for Automated Essay Scoring (AES), yet the scoring rubrics they rely on are typically designed for human raters and may not be optimal for LLMs. Inspired by the calibration process that human raters undergo before formal scoring, we propose Reflect-and-Revise, an iterative framework that refines scoring rubrics by prompting models to reflect on their own chain-of-thought rationales and observed score discrepancies with human labels. At each iteration, the model identifies systematic scoring error patterns from sampled mismatches and revises the rubric accordingly. Experiments on three essay scoring benchmarks (ASAP, ASAP 2.0, and TOEFL11) with three LLMs (GPT-5 mini, Gemini 3 Flash, and Qwen3-80B-A3B) show that our method yields substantial improvements in Quadratic Weighted Kappa (QWK), with gains of up to +0.388 over human-authored rubrics on ASAP. Furthermore, starting from a minimal seed rubric that specifies only the score scale, our method can match or exceed the performance of carefully authored expert rubrics in most settings.\footnote{Optimization and evaluation codes are available at \url{https://anonymous.4open.science/r/ARR2025-1CA3}}
\end{abstract}
...
We evaluate our method on three datasets (ASAP, ASAP 2.0, and TOEFL11) with three models (\texttt{GPT-5 mini}, \texttt{Qwen3-80B-A3B}, and \texttt{Gemini 3 Flash}). 
...
Gemini 3 Flash & \textbf{Ours} & \textbf{0.700} & \textbf{0.540} & \textbf{0.716} & \textbf{0.393} & \textbf{0.520} \\
Gemini 3 Flash & \textbf{Ours from simplest} & \underline{0.651} & 0.470 & \underline{0.682} & \underline{0.310} & 0.580 \\
\midrule
...
To understand how refinement changes rubric content, we analyze the refined rubrics compared to the original human expert rubrics. For quantitative analysis, \autoref{tab:rubric_comparison} presents a word-count-based comparison of the original and refined rubrics. The refined rubrics tend to be longer up to approximately 10 times the length of the original rubrics (GPT-5 mini on TOEFL11). Overlapping words ratio between the original and refined rubrics are about 20\% on average, indicating that the refined rubrics often introduce substantial new content. Especially for GPT-5 mini, overlapping words ratio is as low as 5.1\% on ASAP and leads to a large QWK gain of +0.438 suggesting that the refinement process can discover rubric formulations that are substantially different from human-authored rubrics and better suited for the model's reasoning and scoring process. On the other hand, GPT-5 mini's refined rubric on TOEFL11 has a low overlapping words ratio of 3.3\% but does not improve over the human rubric, suggesting that not all substantial modifications lead to performance gains and have potentials for overfitting to the training set. 

For qualitative analysis, we investigate what kinds of content the refinement process adds to rubrics. We apply regex matching to detect typical patterns in refined rubrics, and categorize the detected patterns into two types (\autoref{fig:pattern_overview}): \textbf{Rule Structure} patterns, which introduce explicit procedural constraints such as conditional gating (e.g., ``if the essay contains fewer than two distinct reasons''), quantitative thresholds, score caps, stepwise workflows, and boundary tie-break guidance; and \textbf{Evidence Handling} patterns, which govern how evidence quality and quantity are assessed, including requirements for concrete exemplification and safeguards against double-counting restatements as distinct support.

\autoref{tab:rubric_pattern_changes} shows the average regex match counts across all three models on three dataset results comparing before and after refinement. The original human-authored rubrics contain almost none of these patterns: the average count is 0.0 for all Rule Structure categories and only 0.3 for Concrete Exemplification. After refinement, all categories show substantial increases. Conditional Gating exhibits the largest gain (+21.2), indicating that the refinement process frequently introduces explicit conditional rules to guide scoring decisions. Concrete Exemplification also increases substantially (+10.7), reflecting the addition of instructions that require models to identify and count specific examples in essays. Quantitative Threshold patterns (+9.1) show that the refined rubrics often specify numeric cutoffs (e.g., ``at least 2 distinct reasons'') to make scoring criteria more precise. These findings suggest that human-authored rubrics, which rely on holistic descriptors, lack the explicit procedural instructions that LLMs need for consistent scoring, and that the refinement process addresses this gap by introducing rule-based decision logic and concrete evidence-assessment guidelines. An example of a full refined rubric with pattern highlights is provided in Appendix~\ref{sec:appendix_refined_rubric}.
...
The following is the full text of the rubric refined by GPT-5~mini on ASAP P1 starting from the expert seed rubric.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{LLM} & \textbf{Baseline} & \textbf{$N=20$} & \textbf{$N=50$} & \textbf{$N=100$} \\
\midrule
Gemini 3 Flash & 0.613 & \textbf{0.724} & 0.619 & \underline{0.700} \\
GPT-5 mini & 0.358 & \textbf{0.544} & \underline{0.540} & 0.537 \\
Qwen3-80B-A3B & 0.575 & \underline{0.616} & 0.557 & \textbf{0.636} \\
\bottomrule
\end{tabular}%
}
\caption{Effect of training sample size ($N$) on test QWK for ASAP 2.0. Baseline uses the human expert rubric without optimization. Bold indicates the best and underline indicates the second-best QWK among training sizes for each model.}
\label{tab:train_size_asap2}
\end{table}
...
% \input{figures/fig_pattern_criteria_examples}
...
\input{figures/pattern_overview}
