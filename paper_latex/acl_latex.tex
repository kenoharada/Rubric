...
\bottomrule
\end{tabular}
\caption{Ablation study on ASAP 2.0. The full method (Ours) outperforms both ablated variants, confirming the importance of both iterative refinement and chain-of-thought rationales for maximizing rubric effectiveness.}
\label{tab:ablation}
\end{table}
...
To quantify the contribution of iterative refinement and chain-of-thought rationales, we compare the full method with two ablated variants: (1) \textbf{w/o Iteration}, which performs only a single revision step ($T=1$), and (2) \textbf{w/o Rationale}, which performs iterative refinement but omits the model-generated rationales from the revision prompt. Table~\ref{tab:ablation} reports the results on ASAP 2.0, where the full method achieves the best QWK for all three models. Table~\ref{tab:ablation_full} in Appendix~\ref{app:ablation} provides the full results across all three datasets.

Overall, the full method achieves the best QWK in seven out of nine dataset--model combinations. Removing rationales reduces QWK in eight out of nine combinations, suggesting that model-generated rationales usually provide useful diagnostic signals for rubric revision. Iterative refinement is also beneficial in seven out of nine combinations, with particularly consistent gains on ASAP 2.0. However, the effect is not uniform: on ASAP with Gemini 3 Flash and on TOEFL11 with GPT-5 mini, the single-step variant outperforms the full iterative procedure. These exceptions suggest that additional refinement can sometimes overfit the small training set or introduce overly model-specific criteria, especially when the initial or early revised rubric is already strong. 
